...
source admin-openrc1.sh $PASSWD
...
if ! [[ ${OPENSTACK_IP1} =~ ${IP_RE} ]]; then
  printf "\033[0;31mThe first Openstack IP is not correct\033[0m\n"
  exit 1
fi

if ! [[ ${OPENSTACK_IP2} =~ ${IP_RE} ]]; then
  printf "\033[0;31mThe second Openstack IP is not correct\033[0m\n"
  exit 1
fi
...
IDENTITY1="http://${OPENSTACK_IP1}/identity"
jq --arg v "${IDENTITY1}" '.builders[].identity_endpoint = $v' imagebuild.json|sponge imagebuild.json
...
source admin-openrc2.sh $PASSWD

IDENTITY2="http://${OPENSTACK_IP2}/identity"
jq --arg v "${IDENTITY2}" '.builders[].identity_endpoint = $v' imagebuild2.json|sponge imagebuild2.json
jq --arg v "${IMAGE_ID}" '.builders[].source_image = $v' imagebuild2.json|sponge imagebuild2.json
jq --arg v "${NETWORK_ID}" '.builders[].networks[] = $v' imagebuild2.json|sponge imagebuild2.json

THE_PATH=$('pwd')
BOOT_SCRIPT="${THE_PATH}/bootscript.sh"
jq --arg v "$BOOT_SCRIPT" '.provisioners[0].source = $v' imagebuild2.json|sponge imagebuild2.json

SERVICE="${THE_PATH}/connectivity.service"
jq --arg v "$SERVICE" '.provisioners[1].source = $v' imagebuild2.json|sponge imagebuild2.json

echo "Building image... This might take some minutes, depending on your hardware and your Internet connection."
packer build imagebuild2.json > /dev/null 2>&1
printf "\033[0;32mCreated image: packerimage.\n\033[0mRun 'openstack image list' for confirmation.\n"
